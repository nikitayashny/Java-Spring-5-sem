...
async function genUserCreateButton(user , Car) {
    let token = localStorage.getItem('token');
...
async function getCertainCarUser(listProjectElement) {
    let token = localStorage.getItem('token');
    ...
    let comp = await userGetCarByName(listProjectElement['name'], token);

    await genCard();
    await genCardCreate(user,comp);
}

async function genListOfCarStuffForUser() {
    let token = localStorage.getItem('token');
    ...
    for (let i = userRents.length - 1; i >= 0; i--) {
        ...
        for (let y = 0; y < 6; y++) {
            ...
            switch (y) {
                ...
                case 2: {
                    th.innerHTML = userRents[i]['car']['name'];
                    break;
                }
                case 3: {
                    th.innerHTML = userRents[i]['car']['expirationDate'];
                    break;
                }
                case 4: {
                ...
                case 5: {
                    ...
                    genBut.onclick = async () => {
                        async function deleteComp(userRentElement) {
                            ...
                            await deleteCarByNameU({name:userRentElement['userName']},token);
                            await genUserInfo();
